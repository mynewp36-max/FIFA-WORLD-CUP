...
exports.buildTransportPrompt = void 0;
...
// ─── Main prompt builder ───
const buildTransportPrompt = (req) => {
    const snippets = [
        metroGuidance,
        busGuidance,
        walkingGuidance,
    ...
    if (req.groupSize > 1)
        snippets.push(groupGuidance(req.groupSize));
    if (req.wheelchair)
        snippets.push(accessibilityGuidance);
    if (req.avoidCrowd)
        snippets.push(avoidCrowdGuidance);
    if (req.ecoFriendly)
        snippets.push(sustainabilityGuidance);
    if (req.weather)
        snippets.push(weatherGuidance(req.weather));
...
exports.buildTransportPrompt = buildTransportPrompt;
